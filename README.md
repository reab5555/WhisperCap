...
# WhisperCap
This is a GUI based tool that uses Whisper model to transcribe video and audio files and create subtitles. there is not duration limitation, and it supports many languages and can automatically determine the video or audio file spoken language.
...
The main advantage of this tool is the ability to bypass the limitation of the transcription time by working in chunks, so video or audio files that are even about an hour long can be transcribed in almost any language. Secondly, the tool is based on a GUI, so it is easy and simple to transcribe files even for those who do not understand how a script works. furthermore, it almost completely eliminates the need for a human to transcribe videos manually, which saves a lot of time and money when one wants to transcribe videos or lectures accurately and efficiently. 
